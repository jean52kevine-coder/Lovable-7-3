  ...
  <Layout>
    <section className="relative py-24 md:py-32 overflow-hidden">
      ...
      <motion.div className="section-container text-center relative z-10" initial={{ opacity: 0, y: 30 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.6 }}>
        ...
        <h1 className="heading-display text-4xl md:text-6xl mb-4">SITE <RotatingWords words={["VITRINE", "PROFESSIONNEL", "SUR-MESURE", "MODERNE"]} /></h1>
        ...
        <div className="relative max-w-2xl mx-auto rounded-2xl overflow-hidden">
          ...
          <div className="relative z-10 rounded-2xl p-8 md:p-10 overflow-hidden" style={{ backgroundColor: "hsl(var(--card-dark))", border: "1px solid hsl(var(--border-green))" }}>
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
              {inclus.map((item, i) => (
                <motion.div key={i} className="flex items-center gap-3" initial={{ opacity: 0, x: -20 }} whileInView={{ opacity: 1, x: 0 }} viewport={{ once: true }} transition={{ duration: 0.4, delay: i * 0.08 }}>
    ...
    <section className="py-[100px]" style={{ backgroundColor: "hsl(var(--hero-bg))" }}>
      <div className="section-container">
        ...
        <div className="flex flex-wrap justify-center gap-4 max-w-3xl mx-auto">
          {cibles.map((c, i) => (
            <motion.div key={i} className="relative rounded-xl" initial={{ opacity: 0, scale: 0.9 }} whileInView={{ opacity: 1, scale: 1 }} viewport={{ once: true }} transition={{ duration: 0.4, delay: i * 0.1 }}>
      ...
      <div className="section-container">
        ...
        <TracingBeam>
          <div className="space-y-8 pl-8 md:pl-12">
            {faqs.map((faq, i) => (
              <motion.div key={i} className="relative rounded-xl" initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ duration: 0.5, delay: i * 0.1 }}>
                <GlowingEffect spread={40} glow proximity={64} inactiveZone={0.01} borderWidth={2} disabled={false} />
