...
const features = [
  {
    title: "Rapidité",
    icon: Zap,
    description: "Livraison en 14 jours chrono. Pendant que d'autres attendent 3 mois, vous êtes déjà en ligne.",
  },
  {
    title: "Design unique",
    icon: Palette,
    description: "Aucun template. Chaque site est conçu sur-mesure pour refléter votre identité.",
  },
  {
    title: "Sécurité",
    icon: Shield,
    description: "SSL, sauvegardes automatiques et monitoring 24/7 pour une tranquillité totale.",
  },
  {
    title: "SEO optimisé",
    icon: TrendingUp,
  ...
  },
  {
    ...
    icon: Clock,
    description: "On s'engage sur une date de livraison et on la tient. Toujours.",
  },
];
  ...
  <Layout>
    ...
    <section className="relative py-24 md:py-32 overflow-hidden">
      ...
      >
        ...
        <p className="font-dm text-lg max-w-2xl mx-auto" style={{ color: "rgba(255,255,255,0.6)" }}>
        ...
        >
          {services.map((s, i) => (
            <motion.div
              key={i}
              variants={fadeUp}
              className="relative rounded-2xl p-7 transition-all duration-300 hover:-translate-y-1.5 group overflow-hidden"
              style={{ backgroundColor: "hsl(var(--card-dark))", border: "1px solid hsl(var(--border-green))" }}
            >
              {s.popular && (
                <>
                  <span className="absolute top-4 right-4 text-[11px] font-bold px-2.5 py-1 rounded-full bg-primary text-primary-foreground z-10">
                    ⭐ POPULAIRE
                  </span>
    ...
    <section className="py-[100px]" style={{ backgroundColor: "hsl(var(--hero-bg))" }}>
      <div className="section-container">
        ...
        >
          ...
          <p className="font-dm text-base text-muted-foreground">
            Tout ce qu'il faut pour réussir en ligne, sans compromis.
          </p>
        </motion.div>
